...
pcall(function()
    for _,g in ipairs(player.PlayerGui:GetChildren()) do
        if g.Name=="LunaAlfian" then g:Destroy() end
    end
end)
...
local TARGET = Vector3.new(-39868.8, 2782.2, -13629.6)
local NEAR   = Vector3.new(-39868.8, 2782.2, -13621.6)
...
local function findChest()
    ...
    for _,v in ipairs(workspace:GetDescendants()) do
        if v:IsA("ProximityPrompt") then
            ...
            if at:match("ambil") or at:match("kode") or at:match("redeem") or
               at:match("claim") or at:match("voucher") or
               pn:match("chest") or pn:match("gopay") then
                return v,v.Parent
            end
        end
    end
...
local function runSequence()
    ...
    task.spawn(function()
        ...
        setStatus("LOCATING CHEST","wait")
        local pr,obj = findChest()
        ...
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification",
                {Title="Mount Luna",Text="Sudah tiba. Ambil kode redeem sekarang.",Duration=3})
        end)
        running = false
    end)
end
...
sg.IgnoreGuiInset  = true   -- TRUE agar muncul di mobile
sg.ZIndexBehavior  = Enum.ZIndexBehavior.Sibling
sg.Parent          = player.PlayerGui  -- langsung ke PlayerGui

-- PANEL — posisi top-left agar pasti kelihatan
local F = Instance.new("Frame", sg)
F.Size             = UDim2.new(0, 220, 0, 152)
F.Position         = UDim2.new(0, 20, 0, 80)  -- pojok kiri atas, di bawah topbar game
...
do
    local fx=Instance.new("Frame",TB)
    fx.Size=UDim2.new(1,0,0,8); fx.Position=UDim2.new(0,0,1,-8)
    fx.BackgroundColor3=DEEP; fx.BorderSizePixel=0; fx.ZIndex=11
end
do
    local bx=Instance.new("Frame",TB)
    bx.Size=UDim2.new(1,0,0,1); bx.Position=UDim2.new(0,0,1,-1)
    bx.BackgroundColor3=BRD; bx.BorderSizePixel=0; bx.ZIndex=12
end
...
TIco.TextYAlignment=Enum.TextYAlignment.Center
...
do
    ...
    l.BackgroundTransparency=1; l.Text="MOUNT LUNA"
...
do
    ...
    l.BackgroundTransparency=1; l.Text="BY ALFIAN"
...
print("Mount Luna | By Alfian | F9 toggle")
