...
pcall(function()
    for _, g in ipairs(player.PlayerGui:GetChildren()) do
        if g.Name == "MountSemut" then g:Destroy() end
    end
end)
...
-- Cara pakai:
-- for _, k in ipairs(koordinat) do
--   hrp.CFrame = CFrame.new(k.x, k.y+5, k.z)
--   task.wait(0.3)
-- end
-- ══════════════════════════════
-- KOORDINAT DARI MOTION LOG
-- Posisi terakhir sebelum prompt
-- t=2.86 -> (-3498.8, 640.3, 376.1)
-- Offset 8 stud agar tidak overlap
-- ══════════════════════════════
local TARGET     = Vector3.new(-34.748, 144.739, 114.876)
local TARGET_NEAR = Vector3.new(-33.748, 141.739, 110.876) -- offset +8 Z
local running  = false
local statusCB = nil
...
-- ══════════════════════════════
-- FIND GOPAY OBJECT
-- obj name dari log = "Gopay"
-- ══════════════════════════════
local function findGopay()
    -- exact name dari log
    for _, v in ipairs(workspace:GetDescendants()) do
        if v.Name == "Gopay" then return v end
    end
    -- case-insensitive fallback
    for _, v in ipairs(workspace:GetDescendants()) do
        local n = v.Name:lower()
        if n:match("gopay") or n:match("redemption") or
...
local function fireAllPrompts(obj)
    ...
    -- scan workspace untuk prompt "Interact" / gopay
    for _, v in ipairs(workspace:GetDescendants()) do
        if v:IsA("ProximityPrompt") then
            local n  = (v.ActionText or ""):lower()
            local pn = (v.Parent and v.Parent.Name or ""):lower()
            if n:match("interact") or n:match("claim") or
               n:match("voucher") or n:match("gopay") or
               pn:match("gopay") or pn:match("redemption") then
                table.insert(candidates, v)
            end
        end
    end
...
local function runSequence()
    ...
    task.spawn(function()
        ...
        -- STEP 2: teleport ke dekat target (offset)
        setStatus("FAST TRAVEL", "wait")
        hrp.CFrame = CFrame.new(TARGET_NEAR + Vector3.new(0, 5, 0))
        task.wait(0.25)

        -- STEP 3: scan objek Gopay
        setStatus("LOCATING GOPAY", "wait")
        local obj = findGopay()
        ...
        -- STEP 4: restore walkspeed lalu fire prompt
        if hum then hum.WalkSpeed = 16 end
        setStatus("FIRING INTERACT", "wait")
        task.wait(0.15)

        local fired = fireAllPrompts(obj)

        -- nudge & retry jika gagal
        if fired == 0 and obj then
            ...
            if pos then
                ...
                task.wait(0.15)
                fired = fireAllPrompts(obj)
            end
        end

        -- STEP 5: selesai
        setStatus("ARRIVED  —  CLAIM MANUAL", "done")
        notif("Mount Semut", "Sudah tiba di GoPay. Claim sekarang.")
        running = false
    end)
end
...
F.Size             = UDim2.new(0, 240, 0, 390)
F.Position         = UDim2.new(0.5, -120, 0.5, -195)
...
TTitle.Text               = "MOUNT SEMUT SCRIPT"
...
local steps = {
    ...
    "2.  Tekan START — teleport ke GoPay",
    "3.  Tunggu status ARRIVED muncul",
    "4.  Klik Interact / Claim 1x saja",
...
-- KOORDINAT INFO
seclbl(9, "KOORDINAT DARI LOG")
local coords = {
    {"TARGET",   string.format("%.1f  %.1f  %.1f", TARGET.X,      TARGET.Y,      TARGET.Z)},
    {"NEAR TP",  string.format("%.1f  %.1f  %.1f", TARGET_NEAR.X, TARGET_NEAR.Y, TARGET_NEAR.Z)},
}
for i, c in ipairs(coords) do
    local row = Instance.new("Frame", Body)
    row.LayoutOrder = 9 + i
    row.Size = UDim2.new(1, 0, 0, 22)
    row.BackgroundColor3 = C2c
    row.BorderSizePixel  = 0
    row.ZIndex = 12
    cr(row, 5) sk(row, C4c, 1)

    local key = Instance.new("TextLabel", row)
    key.Size = UDim2.new(0, 58, 1, 0)
    ...
    val.Size = UDim2.new(1, -66, 1, 0)
    val.Position = UDim2.new(0, 66, 0, 0)
...
StartBtn.ZIndex = 12
cr(StartBtn, 8) sk(StartBtn, C5c, 1)
...
print("Mount Semut Script | By Findme | F9 toggle")
